EESchema Schematic File Version 2
LIBS:power
LIBS:device
LIBS:transistors
LIBS:conn
LIBS:linear
LIBS:regul
LIBS:74xx
LIBS:cmos4000
LIBS:adc-dac
LIBS:memory
LIBS:xilinx
LIBS:microcontrollers
LIBS:dsp
LIBS:microchip
LIBS:analog_switches
LIBS:motorola
LIBS:texas
LIBS:intel
LIBS:audio
LIBS:interface
LIBS:digital-audio
LIBS:philips
LIBS:display
LIBS:cypress
LIBS:siliconi
LIBS:opto
LIBS:atmel
LIBS:contrib
LIBS:valves
LIBS:switches
LIBS:MiscellaneousDevices
LIBS:QuadOpAmp
LIBS:BBD_Short
LIBS:MN3102
LIBS:NE571
LIBS:DualOpAmp
LIBS:Poly-800_mod-cache
EELAYER 25 0
EELAYER END
$Descr User 5354 5708
encoding utf-8
Sheet 5 5
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L SW_DPDT_x2 SW?
U 1 1 59F86E91
P 4100 3150
AR Path="/59F831F3/59F86E91" Ref="SW?"  Part="1" 
AR Path="/59F878A4/59F87971/59F86E91" Ref="SW6"  Part="1" 
AR Path="/59FAABE9/59FAB4E6/59FAE7A0/59F86E91" Ref="SW6"  Part="1" 
F 0 "SW6" H 4000 3050 31  0000 L CNN
F 1 "Chorus-Fb_Invert-B" H 4050 3400 31  0001 C CNN
F 2 "" H 4100 3150 50  0001 C CNN
F 3 "" H 4100 3150 50  0001 C CNN
	1    4100 3150
	1    0    0    -1  
$EndComp
$Comp
L SW_DPDT_x2 SW?
U 1 1 59F86E98
P 4050 1500
AR Path="/59F831F3/59F86E98" Ref="SW?"  Part="1" 
AR Path="/59F878A4/59F87971/59F86E98" Ref="SW3"  Part="1" 
AR Path="/59FAABE9/59FAB4E6/59FAE7A0/59F86E98" Ref="SW3"  Part="1" 
F 0 "SW3" H 4100 1500 31  0000 C CNN
F 1 "Chorus-FilterPan-B" V 4600 1500 31  0001 C CNN
F 2 "" H 4050 1500 50  0001 C CNN
F 3 "" H 4050 1500 50  0001 C CNN
	1    4050 1500
	1    0    0    -1  
$EndComp
$Comp
L SW_DPDT_x2 SW?
U 1 1 59F86E9F
P 4050 900
AR Path="/59F831F3/59F86E9F" Ref="SW?"  Part="1" 
AR Path="/59F878A4/59F87971/59F86E9F" Ref="SW1"  Part="1" 
AR Path="/59FAABE9/59FAB4E6/59FAE7A0/59F86E9F" Ref="SW1"  Part="1" 
F 0 "SW1" H 4100 900 31  0000 C CNN
F 1 "ChorusFilterPan" H 4000 600 31  0000 C CNN
F 2 "" H 4050 900 50  0001 C CNN
F 3 "" H 4050 900 50  0001 C CNN
	1    4050 900 
	1    0    0    -1  
$EndComp
$Comp
L C_Small C?
U 1 1 59F86EA6
P 2700 3000
AR Path="/59F831F3/59F86EA6" Ref="C?"  Part="1" 
AR Path="/59F878A4/59F87971/59F86EA6" Ref="C49"  Part="1" 
AR Path="/59FAABE9/59FAB4E6/59FAE7A0/59F86EA6" Ref="C49"  Part="1" 
F 0 "C49" V 2600 2950 31  0000 L CNN
F 1 "0.1µ" V 2800 2950 31  0000 L CNN
F 2 "" H 2700 3000 50  0001 C CNN
F 3 "" H 2700 3000 50  0001 C CNN
	1    2700 3000
	0    1    1    0   
$EndComp
$Comp
L POT RV?
U 1 1 59F86EAD
P 3050 3650
AR Path="/59F831F3/59F86EAD" Ref="RV?"  Part="1" 
AR Path="/59F878A4/59F87971/59F86EAD" Ref="RV6"  Part="1" 
AR Path="/59FAABE9/59FAB4E6/59FAE7A0/59F86EAD" Ref="RV6"  Part="1" 
F 0 "RV6" H 3200 3550 31  0000 C CNN
F 1 "B10K" V 3050 3650 31  0000 C CNN
F 2 "" H 3050 3650 50  0001 C CNN
F 3 "" H 3050 3650 50  0001 C CNN
	1    3050 3650
	1    0    0    -1  
$EndComp
Text Notes 3300 2950 0    35   ~ 7
Chorus\nWetPass
Text Notes 3550 3700 2    35   ~ 7
Chorus\nFeedback
$Comp
L C_Small C?
U 1 1 59F86EB6
P 2700 3500
AR Path="/59F831F3/59F86EB6" Ref="C?"  Part="1" 
AR Path="/59F878A4/59F87971/59F86EB6" Ref="C51"  Part="1" 
AR Path="/59FAABE9/59FAB4E6/59FAE7A0/59F86EB6" Ref="C51"  Part="1" 
F 0 "C51" V 2650 3550 31  0000 L CNN
F 1 "0.1µ" V 2750 3550 31  0000 L CNN
F 2 "" H 2700 3500 50  0001 C CNN
F 3 "" H 2700 3500 50  0001 C CNN
	1    2700 3500
	0    1    1    0   
$EndComp
Text Notes 4050 2350 2    31   ~ 0
Wet
Text Notes 4250 2450 2    31   ~ 0
Wet-Inverted
Text Notes 4150 1900 2    31   ~ 0
LP Wet
Text Notes 800  2700 0    31   ~ 0
Wet from \nremoved\nC20
Text Notes 4200 1100 0    28   ~ 6
norm
Text Notes 4200 900  0    28   ~ 6
Inv
Text Notes 4200 1700 0    28   ~ 6
Inv
Text Notes 3650 850  0    28   ~ 0
<-Right \nChannel Wet \n(normally HP)
Text Notes 3650 1500 0    28   ~ 0
<-Left\nChannel Wet \n(normally LP)
Text Notes 3600 3150 0    31   ~ 0
<- to LPF (norm.\nLeft) and Fb
Text Notes 3950 2050 0    31   ~ 0
 HP Wet
$Comp
L SW_DPDT_x2 SW?
U 1 1 59F86EC8
P 4100 2650
AR Path="/59F831F3/59F86EC8" Ref="SW?"  Part="1" 
AR Path="/59F878A4/59F87971/59F86EC8" Ref="SW5"  Part="1" 
AR Path="/59FAABE9/59FAB4E6/59FAE7A0/59F86EC8" Ref="SW5"  Part="1" 
F 0 "SW5" H 3950 2550 31  0000 L CNN
F 1 "ChorusFbInv" H 4100 2400 31  0000 C CNN
F 2 "" H 4100 2650 50  0001 C CNN
F 3 "" H 4100 2650 50  0001 C CNN
	1    4100 2650
	1    0    0    -1  
$EndComp
$Comp
L C_Small C?
U 1 1 59F86ECF
P 3450 2650
AR Path="/59F831F3/59F86ECF" Ref="C?"  Part="1" 
AR Path="/59F878A4/59F87971/59F86ECF" Ref="C48"  Part="1" 
AR Path="/59FAABE9/59FAB4E6/59FAE7A0/59F86ECF" Ref="C48"  Part="1" 
F 0 "C48" V 3350 2600 31  0000 L CNN
F 1 "0.1µ" V 3550 2600 31  0000 L CNN
F 2 "" H 3450 2650 50  0001 C CNN
F 3 "" H 3450 2650 50  0001 C CNN
	1    3450 2650
	0    1    1    0   
$EndComp
Text Notes 4200 1500 0    28   ~ 6
norm
Text Notes 4250 2850 0    28   ~ 6
norm
Text Notes 4250 2650 0    28   ~ 6
Inv
Text Notes 4250 3150 0    28   ~ 6
Inv
Text Notes 4250 3350 0    28   ~ 6
norm
Text Notes 2550 3350 1    31   ~ 6
GND
Text Notes 3200 3450 0    31   ~ 0
Not Filtered,\n possibly\n Inverted
Text Notes 2550 3800 0    31   ~ 0
Feedback's \ngain is limited\nby LP filtering
$Comp
L POT_Dual RV?
U 1 1 59F86EDE
P 3050 1400
AR Path="/59F831F3/59F86EDE" Ref="RV?"  Part="1" 
AR Path="/59F878A4/59F87971/59F86EDE" Ref="RV1"  Part="1" 
AR Path="/59FAABE9/59FAB4E6/59FAE7A0/59F86EDE" Ref="RV1"  Part="1" 
F 0 "RV1" V 3050 1350 31  0000 R CNN
F 1 "DUAL_B250K" H 3050 1500 31  0000 C CNN
F 2 "" H 3300 1325 50  0001 C CNN
F 3 "" H 3300 1325 50  0001 C CNN
	1    3050 1400
	0    -1   -1   0   
$EndComp
Text Notes 3250 1450 0    35   ~ 7
Chorus \nStereo \nSeparation
$Comp
L C_Small C?
U 1 1 59F86EE6
P 2850 2050
AR Path="/59F831F3/59F86EE6" Ref="C?"  Part="1" 
AR Path="/59F878A4/59F87971/59F86EE6" Ref="C20"  Part="1" 
AR Path="/59FAABE9/59FAB4E6/59FAE7A0/59F86EE6" Ref="C20"  Part="1" 
F 0 "C20" H 2700 2000 31  0000 L CNN
F 1 "1µ" H 2750 2100 31  0000 C CNN
F 2 "" H 2850 2050 50  0001 C CNN
F 3 "" H 2850 2050 50  0001 C CNN
	1    2850 2050
	-1   0    0    1   
$EndComp
Text Notes 3600 2650 0    31   ~ 0
<- to HPF w/Ω\n(norm. Right)
$Comp
L POT_Dual RV?
U 1 1 59F86EEE
P 3150 2900
AR Path="/59F831F3/59F86EEE" Ref="RV?"  Part="1" 
AR Path="/59F878A4/59F87971/59F86EEE" Ref="RV4"  Part="1" 
AR Path="/59FAABE9/59FAB4E6/59FAE7A0/59F86EEE" Ref="RV4"  Part="1" 
F 0 "RV4" V 3200 2900 31  0000 R CNN
F 1 "DUAL_B100K" H 3150 3000 31  0000 C CNN
F 2 "" H 3400 2825 50  0001 C CNN
F 3 "" H 3400 2825 50  0001 C CNN
	1    3150 2900
	0    1    1    0   
$EndComp
$Comp
L C_Small C?
U 1 1 59F86EF5
P 1650 2000
AR Path="/59F831F3/59F86EF5" Ref="C?"  Part="1" 
AR Path="/59F878A4/59F87971/59F86EF5" Ref="C11"  Part="1" 
AR Path="/59FAABE9/59FAB4E6/59FAE7A0/59F86EF5" Ref="C11"  Part="1" 
F 0 "C11" H 1650 2050 31  0000 L CNN
F 1 "0.1µ" H 1650 1950 31  0000 L CNN
F 2 "" H 1650 2000 50  0001 C CNN
F 3 "" H 1650 2000 50  0001 C CNN
	1    1650 2000
	-1   0    0    -1  
$EndComp
$Comp
L C_Small C?
U 1 1 59F86EFC
P 1200 2300
AR Path="/59F831F3/59F86EFC" Ref="C?"  Part="1" 
AR Path="/59F878A4/59F87971/59F86EFC" Ref="C17"  Part="1" 
AR Path="/59FAABE9/59FAB4E6/59FAE7A0/59F86EFC" Ref="C17"  Part="1" 
F 0 "C17" H 1250 2200 31  0000 L CNN
F 1 "0.1µ" H 1250 2400 31  0000 C TNN
F 2 "" H 1200 2300 50  0001 C CNN
F 3 "" H 1200 2300 50  0001 C CNN
	1    1200 2300
	-1   0    0    1   
$EndComp
$Comp
L R R?
U 1 1 59F86F0A
P 2250 2500
AR Path="/59F831F3/59F86F0A" Ref="R?"  Part="1" 
AR Path="/59F878A4/59F87971/59F86F0A" Ref="R13"  Part="1" 
AR Path="/59FAABE9/59FAB4E6/59FAE7A0/59F86F0A" Ref="R13"  Part="1" 
F 0 "R13" H 2300 2650 31  0000 C CNN
F 1 "47K" V 2250 2500 31  0000 C CNN
F 2 "" V 2180 2500 50  0001 C CNN
F 3 "" H 2250 2500 50  0001 C CNN
	1    2250 2500
	1    0    0    -1  
$EndComp
$Comp
L CP1_Small C?
U 1 1 59F86F11
P 1350 2300
AR Path="/59F831F3/59F86F11" Ref="C?"  Part="1" 
AR Path="/59F878A4/59F87971/59F86F11" Ref="C21"  Part="1" 
AR Path="/59FAABE9/59FAB4E6/59FAE7A0/59F86F11" Ref="C21"  Part="1" 
F 0 "C21" H 1250 2400 31  0000 L CNN
F 1 "1µ" H 1300 2250 31  0000 C TNN
F 2 "" H 1350 2300 50  0001 C CNN
F 3 "" H 1350 2300 50  0001 C CNN
	1    1350 2300
	1    0    0    -1  
$EndComp
$Comp
L CP1_Small C?
U 1 1 59F86F18
P 1500 2000
AR Path="/59F831F3/59F86F18" Ref="C?"  Part="1" 
AR Path="/59F878A4/59F87971/59F86F18" Ref="C7"  Part="1" 
AR Path="/59FAABE9/59FAB4E6/59FAE7A0/59F86F18" Ref="C7"  Part="1" 
F 0 "C7" H 1550 2050 31  0000 L CNN
F 1 "1µ" H 1550 1950 31  0000 L CNN
F 2 "" H 1500 2000 50  0001 C CNN
F 3 "" H 1500 2000 50  0001 C CNN
	1    1500 2000
	-1   0    0    -1  
$EndComp
Text HLabel 950  2750 0    39   Input ~ 0
Wet
Text Notes 3700 700  0    35   ~ 0
Point to Point wiring right\nof Chorus-Control PCB\n
Text Notes 2500 700  0    35   ~ 0
PCB mounted pots on\nChorus-Control PCB
Text Notes 800  1350 0    39   ~ 0
Chorus-Control PCB
Text Notes 950  1550 0    35   ~ 0
mounted to synth's upper right edge\nvia pots attachement to the synth
Text Notes 2550 1500 1    31   ~ 6
GND
$Comp
L R R12
U 1 1 5A1722C6
P 2100 2500
F 0 "R12" H 2150 2650 31  0000 C CNN
F 1 "47K" V 2100 2500 31  0000 C CNN
F 2 "" V 2030 2500 50  0001 C CNN
F 3 "" H 2100 2500 50  0001 C CNN
	1    2100 2500
	1    0    0    -1  
$EndComp
Wire Wire Line
	4350 1900 2850 1900
Wire Wire Line
	4450 2050 3050 2050
Wire Wire Line
	3550 2650 3900 2650
Wire Notes Line
	4000 1400 4000 950 
Wire Wire Line
	2850 1900 2850 1950
Wire Wire Line
	2850 2150 2850 3000
Wire Wire Line
	3050 2050 3050 2500
Connection ~ 2850 3000
Wire Wire Line
	2800 3000 2850 3000
Wire Wire Line
	2850 3000 3050 3000
Connection ~ 4400 2750
Connection ~ 4500 2550
Wire Wire Line
	4350 1600 4250 1600
Wire Wire Line
	4350 800  4350 1600
Wire Wire Line
	4350 1600 4350 1900
Wire Wire Line
	4250 800  4350 800 
Wire Wire Line
	4450 1400 4250 1400
Wire Wire Line
	4450 1000 4450 1400
Wire Wire Line
	4450 1400 4450 2050
Wire Wire Line
	4250 1000 4450 1000
Wire Wire Line
	4500 3250 4300 3250
Wire Wire Line
	4500 2300 4500 2550
Wire Wire Line
	4500 2550 4500 3250
Wire Wire Line
	4300 2550 4500 2550
Wire Wire Line
	4400 3050 4300 3050
Wire Wire Line
	4400 2450 4400 2750
Wire Wire Line
	4400 2750 4400 3050
Wire Wire Line
	4400 2750 4300 2750
Wire Wire Line
	2400 2450 4400 2450
Wire Wire Line
	2550 3500 2600 3500
Wire Notes Line
	4100 2650 4100 3050
Wire Wire Line
	950  2750 1500 2750
Wire Wire Line
	3200 3650 3300 3650
Connection ~ 4450 1400
Connection ~ 4350 1600
Wire Wire Line
	3150 1800 2550 1800
Wire Wire Line
	2550 1300 2550 1800
Wire Wire Line
	2550 1800 2550 2800
Wire Wire Line
	2550 2800 2550 3000
Wire Wire Line
	2550 3000 2550 3500
Wire Wire Line
	2550 3000 2600 3000
Wire Wire Line
	2550 2800 3050 2800
Connection ~ 2550 3000
Wire Wire Line
	3150 1300 2550 1300
Connection ~ 2550 1800
Connection ~ 2550 2800
Wire Wire Line
	2800 3500 2900 3500
Wire Wire Line
	2900 3500 3050 3500
Wire Wire Line
	2900 3700 2900 3500
Connection ~ 2900 3500
Wire Wire Line
	3350 2650 3250 2650
Wire Wire Line
	1450 2300 2250 2300
Wire Wire Line
	2250 2300 4500 2300
Wire Wire Line
	3250 3150 3300 3150
Wire Wire Line
	3300 3150 3900 3150
Connection ~ 3300 3150
Wire Wire Line
	1500 2550 1500 2600
Wire Wire Line
	1500 2600 1500 2650
Wire Wire Line
	950  2150 1050 2150
Wire Wire Line
	1050 2150 1200 2150
Wire Wire Line
	1200 2150 1350 2150
Wire Wire Line
	1350 2150 1500 2150
Wire Wire Line
	1500 2150 1650 2150
Wire Wire Line
	1900 2650 2000 2650
Wire Wire Line
	2000 2650 2400 2650
Wire Wire Line
	1900 2750 2100 2750
Wire Wire Line
	2100 2750 2250 2750
Wire Wire Line
	1350 2200 1350 2150
Wire Wire Line
	1200 2200 1200 2150
Connection ~ 1350 2150
Wire Wire Line
	1350 2400 1350 2450
Wire Wire Line
	1350 2450 1350 2850
Wire Wire Line
	950  2450 1200 2450
Wire Wire Line
	1200 2450 1350 2450
Wire Wire Line
	1200 2400 1200 2450
Wire Wire Line
	1350 2850 1500 2850
Connection ~ 1200 2150
Wire Wire Line
	1500 2600 1450 2600
Wire Wire Line
	1450 2600 1450 2300
Wire Wire Line
	2250 2300 2250 2350
Connection ~ 1500 2600
Wire Wire Line
	3850 1500 3150 1500
Wire Wire Line
	2400 2650 2400 2450
Wire Wire Line
	2900 1000 3150 1000
Wire Wire Line
	2900 1150 2950 1150
Wire Wire Line
	2950 1650 2900 1650
Wire Notes Line
	4600 550  4600 3400
Wire Notes Line
	4600 3400 3600 3400
Wire Notes Line
	3600 550  3600 3900
Wire Notes Line
	2350 550  4600 550 
Wire Notes Line
	3600 3900 2350 3900
Wire Notes Line
	2350 3900 2350 550 
Wire Wire Line
	1250 1850 1500 1850
Wire Wire Line
	1500 1850 1500 1900
Wire Wire Line
	1500 1850 1650 1850
Wire Wire Line
	1650 1850 1900 1850
Wire Wire Line
	2500 3700 2900 3700
Wire Notes Line
	2350 1250 750  1250
Wire Notes Line
	750  3350 2350 3350
Wire Wire Line
	3800 900  3850 900 
Wire Wire Line
	3300 3650 3300 3150
Wire Wire Line
	1900 2950 1900 2850
Wire Wire Line
	1900 1850 1900 2550
Connection ~ 1500 1850
Wire Wire Line
	1650 1900 1650 1850
Connection ~ 1650 1850
Wire Wire Line
	1650 2150 1650 2100
Wire Wire Line
	1500 2100 1500 2150
Connection ~ 1500 2150
Connection ~ 1350 2450
Connection ~ 1200 2450
Wire Wire Line
	2250 2750 2250 2650
Wire Wire Line
	2100 2650 2100 2750
Connection ~ 2100 2750
Wire Wire Line
	2100 2350 2000 2350
Wire Wire Line
	2000 2350 2000 2650
Connection ~ 2000 2650
Connection ~ 2250 2300
$Comp
L DualOpAmp U2
U 1 1 5A17A57D
P 1700 2700
F 0 "U2" H 1700 2600 31  0000 C CNN
F 1 "NJM4556" H 1700 2800 31  0000 C CNN
F 2 "MODULE" H 1700 2700 25  0001 C CNN
F 3 "DOCUMENTATION" H 1700 2700 25  0001 C CNN
	1    1700 2700
	1    0    0    -1  
$EndComp
Wire Notes Line
	750  1250 750  3350
Text Notes 2600 2200 0    31   ~ 0
Replaces\nKorg's \nC20,\nno need \nfor it on\nLP
Text HLabel 2900 1000 0    39   Input ~ 0
Dry
Text HLabel 2900 1650 0    39   Output ~ 0
WetL
Text HLabel 2900 1150 0    39   Output ~ 0
DryR
Text HLabel 3800 900  0    39   Output ~ 0
WetR
Text HLabel 1250 1850 0    39   Input ~ 0
+A
Text HLabel 950  2150 0    39   Input ~ 0
GND
Text HLabel 950  2450 0    39   Input ~ 0
-A
Text HLabel 2500 3700 0    39   Output ~ 0
FB
Wire Wire Line
	1050 2150 1050 2950
Wire Wire Line
	1050 2950 1900 2950
Connection ~ 1050 2150
$EndSCHEMATC
